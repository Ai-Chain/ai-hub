...
from aihubspawner import spawner_options, utils
...
class MLHubDockerSpawner(DockerSpawner):
    ...
    def connect_hub_to_network(self, network):
        try:
            ...
            # Example 403 error:
            # 403 Client Error: Forbidden ("endpoint with name aihub already exists in network jupyter-admin")
            if e.status_code != 403:
                self.log.error(
                    "Could not connect aihub to the network and, thus, cannot create the container.")
                return
    ...
    def is_update_available(self):
        try:
            # compare the last parts of the images, so that also "aichain/ai-workspace:0.8.7 = ai-workspace:0.8.7" would match
