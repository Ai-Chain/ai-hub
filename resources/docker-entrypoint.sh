...
if [ "$execution_mode" == "k8s" ]; then
  ...
  if ! echo $HUB_NAME | pcregrep "^(?![0-9]+$)(?!-)[a-zA-Z0-9-]{1,5}(?<!-)$" > /dev/null; then
    echo "Container name for ai-hub is either too long or not DNS-compatible. Make sure that a DNS-compatible name (--env HUB_NAME) with 1 to 5 characters is provided for the ai-hub container."
    exit 1
  fi
fi
...
function start_ssh {
    ...
    export SSH_TARGET_LABELS="aihub.origin=$HUB_NAME"
